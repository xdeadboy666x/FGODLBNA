...
class User:
    ...
    def topLogin(self):
        ...
        userState = (-int(lastAccessTime) >> 2) ^ int(self.user_id_) & fgourl.data_server_folder_crc_
        ...
        ticket = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 4001), 0)
        goldenfruit = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 100), 0)
        silverfruit = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 101), 0)
        bronzefruit = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 102), 0)
        bluebronzesapling = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 103), 0)
        bluebronzefruit = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 104), 0)
        pureprism = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 46), 0)
        sqf01 = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 16), 0)
        holygrail = next((item['num'] for item in data['cache']['replaced']['userItem'] if item['itemId'] == 7999), 0)
        ...
        login = Login(self.name_, login_days, total_days, act_max, act_recover_at, now_act, add_fp, total_fp)
        ...
        if 'seqLoginBonus' in data['response'][0]['success']:
            ...
            items = [f"{i['name']} x{i['num']}" for i in data['response'][0]['success']['seqLoginBonus'][0]['items']]
            ...
            if 'campaignbonus' in data['response'][0]['success']:
                ...
                items_camp_bonus = [f"{i['name']} x{i['num']}" for i in data['response'][0]['success']['campaignbonus'][0]['items']]
            else:
                bonus_name = None
                bonus_detail = None
                items_camp_bonus = []
    ...
    def buyBlueApple(self, quantity=1):
        ...
        if 'response' in data:
            for response in data['response']:
                ...
                resSuccess = response.get('success')
        ...
        main.logger.info(f"{quantity}x Blue Apple purchased!")
        webhook.shop("Blue Apple", quantity)
        ...
        for response in responses:
            ...
            if "gachaInfos" in resSuccess:
                for info in resSuccess['gachaInfos']:
                    servantArray.append(gacha.gachaInfoServant(info['isNew'], info['objectId'], info['sellMana'], info['sellQp']))

            if "eventMissionAnnounce" in resSuccess:
                for mission in resSuccess["eventMissionAnnounce"]:
                    missionArray.append(gacha.EventMission(mission['message'], mission['progressFrom'], mission['progressTo'], mission['condition']))
    ...
    def lq002(self):
        ...
        present_ids = [str(item['presentId']) for item in data['cache']['replaced']['userPresentBox'] if item['objectId'] in [2, 6, 11, 16, 3, 46, 18, 48, 4001, 100, 101, 102, 103, 104, 1, 4, 7998, 7999, 1000, 2000]]
...
user_obj = User(user_id, auth_key, secret_key)
user_obj.topLogin()
