...
class user:
    ...
    def _login(self, endpoint):
        ...
        data = self.Post(f'{fgourl.server_addr_}/login/{endpoint}?_userId={self.user_id_}')
        ...
        rewards = self._parse_rewards(data['cache']['replaced']['userItem'])
        DataWebhook.append(rewards)

        login = self._parse_login(data)
        DataWebhook.append(login)

        if 'seqLoginBonus' in data['response'][0]['success']:
            bonus = self._parse_bonus(data['response'][0]['success'])
    ...
    def _parse_rewards(self, user_items):
        item_map = {item['itemId']: item['num'] for item in user_items}
        return Rewards(
            stone=item_map.get(4001, 0),
            level=item_map.get(4001, 0),  # replace with correct level extraction
            ticket=item_map.get(4001, 0),
            goldenfruit=item_map.get(100, 0),
            silverfruit=item_map.get(101, 0),
            bronzefruit=item_map.get(102, 0),
            bluebronzesapling=item_map.get(103, 0),
            bluebronzefruit=item_map.get(104, 0),
            pureprism=item_map.get(46, 0),
            sqf01=item_map.get(16, 0),
            holygrail=item_map.get(7999, 0)
        )

    def _parse_login(self, data):
        user_game = data['cache']['replaced']['userGame'][0]
        login_info = data['cache']['updated']['userLogin'][0]
        return Login(
            name=self.name_,
            login_days=login_info['seqLoginCount'],
            total_days=login_info['totalLoginCount'],
            act_max=user_game['actMax'],
            act_recover_at=user_game['actRecoverAt'],
            now_act=user_game['actMax'] - (user_game['actRecoverAt'] - mytime.GetTimeStamp()) / 300,
            add_fp=data['response'][0]['success']['addFriendPoint'],
            total_fp=data['cache']['replaced']['tblUserGame'][0]['friendPoint']
        )

    def _parse_bonus(self, success_response):
        bonus_info = success_response['seqLoginBonus'][0]
        items = [f'{i["name"]} x{i["num"]}' for i in bonus_info['items']]
        items_camp_bonus = []

        if 'campaignbonus' in success_response:
            campaign = success_response['campaignbonus'][0]
            items_camp_bonus = [f'{i["name"]} x{i["num"]}' for i in campaign['items']]
            return Bonus(bonus_info['message'], items, campaign['name'], campaign['detail'], items_camp_bonus)
        
        return Bonus(bonus_info['message'], items, None, None, items_camp_bonus)

    # Remaining methods (buyBlueApple, drawFP, etc.) go hereclass user:
    def __init__(self, user_id: str, auth_key: str, secret_key: str):
        self.name_ = ''
        self.user_id_ = (int)(user_id)
        self.s_ = fgourl.NewSession()
